...
for (species_name in species_list) {
  ...
  p <- predict(fit, newdata = pred_grid, type = "response")
  saveRDS(p, file = paste0(result_dir, "/predictions.RDS"))
  cat("Complete\n")

  cat("Simulating densities over interpolation grid...")
  sim_data <- predict(fit, newdata = pred_grid, type = "response", nsim = 100)
