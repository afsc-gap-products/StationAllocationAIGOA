...
simulate_goa_strs <- function(d_gct = NULL,
  ...
  strata_names <- stn_allocation$stratum
  ...
  mean_stats <- var_stats <-
    array(dim = c(n_strata, n_species, n_years),
          dimnames = list(stratum_info$stratum, species_names, NULL))

  for (iyear in 1:n_years) {
    for (ii in 1:n_strata) { #Loop over strata -- start
      #For each stratum ii, randomly sample stations based on the allocation
      #and populate "strata.stats" with the SRS mean, sd, CV
      stratum_stats <-
        sample_srs(d_gct = d_gct[grid_df$STRATUM == stratum_info$stratum[ii],
                             ,
                             iyear],
               sample_size = stratum_info$n[ii])
